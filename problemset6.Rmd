...
## Part 1: Background
#1
Having more kids can effect labour supply of the parents, but a causal link from labour supply to the number of children is also theoretically very plausible. Potential selection problem: Women who specialize in housework for various reasons other than having more children (unobserved wage rate, preferences, value system etc.), get more children. 
 


#2
Randomly assign numbers of children to couples at all levels of hours worked. Obviously it is not feasible to make couples have a certain number of kids against their will. Further issue could be that couples  behave differently in the experimental setup. This would flaw the results. 

#3

The authors claim that both instruments induce an exogenous variation in family size. 

samesex: dummy variable, indicating whether the first two children are of the same sex or not. As gender is randomly assigned, parents' labour supply should be uncorrelated with the samesex variable, thus the exclusion restriction holds. However, as parents have a well-reported preference for a mixed sibling-sex composition, parents of same-sex siblings are significantly more likely to go on and have further children, thus samesex effects the variable "More than two kids" and the relevance restriction holds.
...
Debatable: In the twin case, we may argue that parents with in-vitro fertilization have a higher chance of getting twins or triplets, but parents with in-vitro fertilization may differ in other important aspects such as preference to work, unobserved opportunity costs of housework, etc. However, as the data are rather old - in-vitro fertilizations may not have been that common and the exclusion restrictio still holds.
...
#6
(1)	OLS estimates qualitatively correct, however, somewhat exaggerate the negative of children on the female labor supply.
(2)	The effect is heterogenous. Causal effect on college-educated women and women with husbands with high wages is absent.
(3)	Effect on male labor supply is negligible. Men do not change their work habits. If they contribute more to housework, they decrease their leisure time.
(4)	No evidence that income losses due to reduction in labor supply of women are offset by increases in male labor earnings.
(5) The comparison of the sibling sex approach and the twins approach shows that the impact of having more than two children disappears when the third child reaches age 13.

#7
External Validity: 

•	Restricted to US. No general insights. Different value systems, institutional settings may give different results.
•	Time period is 1980s and 1990s. By now, causal effects may look quite different.
•	Causal effect on compliers. Is this an interesting subpopulation? Maybe not.

Internal validity:  

•	Instruments are not well-suited, exlusion restriction violated etc.

(Relevance of samesex may not be guaranteed.

Food for thought: Relevance can be tested? Exclusion restriction)  
...
skim(df)
```
Values look reasonable. Some very extreme values (hoursworked 99, kidcount 12, weekly labor income) but very few observations. No missings
Low share of blacks and hispanics? 
...
robust.se(ivjoint)
```
 With multiple instruments the newly estimated covariate is an average of covariate-specific LATEs. As an average it must be a convex combination of the covariates.
...
```
Degrees of freedom is m - k = 2 (restrictions 2) - 1 (number of endogenous regressors) !?!? 
#11
```{r}
...
We should not include income as labor supply will also determine income (again endogeneity). Best approach find some exogenous variation in labor income: e.g. collectively bargained wages etc. 
